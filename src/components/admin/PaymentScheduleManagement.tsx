      ...
      const scheduleRecords: PaymentScheduleRecord[] = receipts
        ...
        .map(receipt => {
          ...
          return {
            ...
            sender_name: senderDetails?.name || `${senderDetails?.firstName || ''} ${senderDetails?.lastName || ''}`.trim() || 'N/A',
            sender_email: senderDetails?.email || 'N/A',
            sender_phone: senderDetails?.phone || 'N/A',
        ...
        <CardContent>
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="relative flex-grow">
              <Input
                placeholder="Search by name, email, phone, or receipt number..."
                className="pl-10"
                value={searchQuery}
                      ...
                      <TableCell>
                        <div className="space-y-1">
                          {record.payment_schedule.map((payment, index) => (
                            <div 
                              key={index} 
                              className={`text-sm flex justify-between items-center p-1 rounded ${
