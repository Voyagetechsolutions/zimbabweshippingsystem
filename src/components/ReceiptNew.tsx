...
const ReceiptNew: React.FC<ReceiptProps> = ({ 
    ...
    const createReceiptRecord = async () => {
      if (bookingData && paymentData && !propReceipt && bookingData.shipment_id) {
        try {
          ...
          toast({
            title: "Receipt Error",
            description: "There was an issue saving your receipt. Please contact support.",
            variant: "destructive",
          });
        }
      }
  ...
  const handleTrackShipment = () => {
    ...
    if (trackingNumber && trackingNumber !== 'Pending') {
      ...
      toast({
        ...
        description: "Tracking number is not available yet. Please check back later.",
      });
    }
  };
  ...
  return (
    <div className="container mx-auto px-2 sm:px-4 max-w-4xl mb-8">
              ...
              <table className="w-full min-w-[400px]">
                ...
                <tbody>
                  <tr className="border-t">
                    ...
                    <td className="p-2 sm:p-3 text-xs sm:text-sm">{shipment.status || receipt.status || 'Processing'}</td>
                  </tr>
                </tbody>
              </table>
            </div>
