...
function createTwilioSock(client, fromNumber) {
  return {
    sendMessage: async (to, content) => {
      ...
      let twilioTo = to;
      if (to.includes('@')) {
        // Extract phone number from JID format (e.g., 447123456789@s.whatsapp.net)
...
app.post('/webhook', async (req, res) => {
  try {
    ...
    const fromNumber = req.body.From; // Format: whatsapp:+447123456789
...
app.get('/health', (req, res) => {
  res.json({
    status: 'ok',
    service: 'Zimbabwe Shipping WhatsApp Bot UK (Twilio)',
...
app.get('/', (req, res) => {
  res.send(`
    <html>
      <head><title>Zimbabwe Shipping WhatsApp Bot UK</title></head>
      <body style="font-family: Arial; padding: 40px; max-width: 600px; margin: 0 auto;">
        <h1>🇬🇧 Zimbabwe Shipping WhatsApp Bot UK</h1>
...
(async () => {
  try {
    console.log('🚀 Starting Zimbabwe Shipping WhatsApp Bot UK (Twilio)...');
