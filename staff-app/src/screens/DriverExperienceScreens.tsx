...
import { ActivityIndicator, Image, Linking, Platform, Pressable, RefreshControl, ScrollView, StyleSheet, Switch, Text, TextInput, View } from 'react-native';
...
export function DriverStopDetailsScreen({ route, navigation }: NativeStackScreenProps<DriverRunStackParams, 'StopDetails'>) {
  ...
  const loadStop = useCallback(async (candidateStopId?: string | null) => {
    ...
    if (result.data) {
      ...
      const shipmentResult = stop.kind === 'collection'
        ? await supabase.rpc('driver_collection_detail', { p_shipment_id: stop.shipmentId })
        : await supabase.from('shipments').select('*').eq('id', stop.shipmentId).maybeSingle();
  ...
  const advance = async () => {
    ...
    if (stop.kind === 'collection') {
      setBusy(true);
      try {
        const { data, error } = await supabase.rpc('begin_driver_pickup', { p_shipment_id: stop.shipmentId });
        if (error) throw error;
        if (!data?.stopId) throw new Error('The collection did not start. Please refresh and retry.');
        setResolvedStopId(data.stopId);
        navigation.navigate('StopWorkflow', { stop: { ...stop, id: data.stopId } });
      } catch (e: any) {
        setActionError(isMissingBackend(e) ? 'The office needs to apply the driver database update.' : e?.message || 'Could not start collection. Check your connection.');
  ...
  const actionLabel = stop.kind === 'collection' ? (row?.status === 'completed' ? 'Collected' : row?.status === 'arrived' ? 'Verify details & collect' : 'I’m here · start collection') : row?.status === 'planned' ? 'Start journey' : row?.status === 'en_route' ? 'Mark arrived' : row?.status === 'arrived' ? 'Verify details & collect' : 'Completed';
