...
<head>
  ...
  <title>Zimbabwe Shipping — UK Bot</title>
  ...
  <style>
    ...
    a.download { display:inline-block; margin-top:12px; background:#10b981; color:#fff; padding:10px 18px; border-radius:8px; text-decoration:none; font-weight:600; font-size:14px; }
  </style>
</head>
<body>
  <div class="card">
    <h1>🇬🇧 Zimbabwe Shipping — UK Bot</h1>
    ${body}
...
async function handleRequest(req, res) {
  ...
  if ((req.url === '/qr.png' || req.url?.startsWith('/qr.png?')) && currentQr) {
    try {
      const buf = await QRCode.toBuffer(currentQr, { width: 360, margin: 2 });
      res.writeHead(200, {
        'Content-Type': 'image/png',
        'Cache-Control': 'no-store',
        'Content-Disposition': 'inline; filename="zimship-uk-qr.png"',
      });
      res.end(buf);
      return;
    } catch (err) {
  ...
  if (req.url === '/qr.png/download' && currentQr) {
    try {
      const buf = await QRCode.toBuffer(currentQr, { width: 720, margin: 2 });
      res.writeHead(200, {
        'Content-Type': 'image/png',
        'Cache-Control': 'no-store',
        'Content-Disposition': 'attachment; filename="zimship-uk-qr.png"',
      });
      res.end(buf);
      return;
    } catch (err) {
  ...
  if (connectedUser) {
    ...
    body = `
      ...
      <a class="download" href="/qr.png/download">⬇️ Download QR Code</a>
    `;
  } else {
